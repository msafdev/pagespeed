...
export class ExportService {
  ...
  private generateAsciiSummary(results: Result[]): string {
    ...
    const formatScores = (scores: Record<string, number | 'N/A'>) => {
      return Object.entries(scores)
        .map(([key, val]) => {
          const label = `"${key}"`.padEnd(18);
          const score = typeof val === 'number' ? val : 0;
          const barComment = `// ${bar(score)}`;
          return `      ${label}: ${score
            .toString()
            .padStart(3)}, ${barComment}`;
        })
        .join('\n');
    };

    const entries = results.map((r) => {
      const lang = 'en'; // You can enhance this to detect real language
      const scoresBlock = formatScores(r.scores);
      return `  "${r.url}": {\n    strategy: "${r.strategy}",\n    language: "${lang}",\n    scores: {\n${scoresBlock}\n    }\n  }`;
    });
  ...
  async injectResultsToReadme(
    ...
    const startTag = '<!-- PAGESPEED_START -->';
    const endTag = '<!-- PAGESPEED_END -->';

    const newSection = this.generateAsciiSummary(results);

    const injected = `${startTag}\n${newSection}\n${endTag}`;

    const updatedReadme =
      readmeContent.includes(startTag) && readmeContent.includes(endTag)
    ...
    console.log(
      chalk.green(`\n✅ Injected PageSpeed results into ${readmePath}`),
    );
